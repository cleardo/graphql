...
from graphql_query import mainclass

# 批量注册
class BatchEnroll(mainclass):
    ...
    def enroll(self, serialNumber, panelName):
        ...
        """
        response = self.send_query(self.url, enroll % (serialNumber, panelName), self.headers)
        body = json.loads(response.text)
        print (enroll)
        return response, body
    ...
    def bulk_enroll_validation(self, filename):
        final_string = ""
    ...
    def bulk_unenroll(self,filename):
        final_string = ""
        dir = os.path.dirname(os.path.abspath(batch_enroll_data.REPORT_DIR))  # 获取csv文件的路径
        data_filename = os.path.join(dir, filename)  # 获取csv文件
        with open(data_filename) as f:
